...
entry_queue/waiting_vehicles/vehicles_in_network/movement_demand/peek_flows/
remaining_real_route/set_route/inject/cumulative_inflow/cumulative_outflow`; attr `step`.
...
`peek_flows(node_id, supply_overrides=None) -> dict[int, list[VehicleView]]`:
predicted ACTUAL crossings per outbound link this step (read-only replay of the
node's flow algorithm; `supply_overrides` replaces receiving flows in the replay).
